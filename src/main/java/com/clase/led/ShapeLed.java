...
public class ShapeLed extends Region {
    ...
    private Color antiguoColor;

    //3. Añade una propiedad “borderColor” que permita elegir el color
    //del borde del led
    //Este color es el que selecciona para cambiar el borde
...
//        getChildren().add(canvas);
    ...
    private void registerListeners() {
        ...
        this.setOnMouseEntered((MouseEvent evt) -> {
            antiguoColor = ledColorProperty().get();
            this.setLedColor(Color.rgb((int) (Math.random() * 255) + 1,
                    (int) (Math.random() * 255) + 1,
                    (int) (Math.random() * 255) + 1));
        });
        this.setOnMouseExited((MouseEvent evt) -> {
            this.setLedColor(antiguoColor);
        });
    }
